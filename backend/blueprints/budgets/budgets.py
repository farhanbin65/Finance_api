...
def addBudget(user_id):
    ...
    # ← removed category_id from required fields
    if not data or "budget_amount" not in data or "month" not in data:
    ...
    try:
        ...
        budgets = user.get("monthly_budgets", [])
        new_budget_id = max((b.get("budget_id", 0) for b in budgets), default=0) + 1
